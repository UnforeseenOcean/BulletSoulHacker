...
public class MasterAudioManager : EditorWindow {
	...
	void OnGUI() {
		...
		Texture header = (Texture) AssetDatabase.LoadAssetAtPath(MasterAudioFolderPath + "/Sources/Textures/inspector_header_master_audio.png", typeof(Texture));
		if (header != null) {
			GUIHelper.ShowHeaderTexture(header);
		}
        Texture settings = (Texture)AssetDatabase.LoadAssetAtPath(MasterAudioFolderPath + "/Sources/Textures/gearIcon.png", typeof(Texture));
	...
	private void CreateMasterAudio() {
        var ma = AssetDatabase.LoadAssetAtPath(MasterAudioFolderPath + "/Prefabs/MasterAudio.prefab", typeof(GameObject));
		if (ma == null) {
	...
	private void CreatePlaylistController() {
        var pc = AssetDatabase.LoadAssetAtPath(MasterAudioFolderPath + "/Prefabs/PlaylistController.prefab", typeof(GameObject));
		if (pc == null) {
	...
	private void CreateDynamicSoundGroupCreator() {
        var pc = AssetDatabase.LoadAssetAtPath(MasterAudioFolderPath + "/Prefabs/DynamicSoundGroupCreator.prefab", typeof(GameObject));
		if (pc == null) {
