...
public class MasterAudioInspector : Editor {
						...
						case GUIHelper.DTFunctionButtons.Play:
							if (Application.isPlaying) {
								...
								if (rndVar.audLocation == MasterAudio.AudioLocation.ResourceFile) {
									...
									MasterAudio.PreviewerInstance.PlayOneShot(Resources.Load(rndVar.resourceFileName) as AudioClip, rndVar.GetComponent<AudioSource>().volume);
								} else {
									rndVar.GetComponent<AudioSource>().Play();
								}
							}
							break;
						case GUIHelper.DTFunctionButtons.Stop:
							if (Application.isPlaying) {
								...
								for (var i = 0; i < aGroup.groupVariations.Count; i++) {
									aGroup.groupVariations[i].GetComponent<AudioSource>().Stop();
	...
	private void CreateVariation(Transform groupTrans, AudioClip aClip) {
		...
		if (sounds.bulkLocationMode == MasterAudio.AudioLocation.ResourceFile) {
			...
			newVariation.GetComponent<AudioSource>().clip = aClip;
		}
