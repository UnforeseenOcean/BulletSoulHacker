...
public class MasterAudio : MonoBehaviour {
	...
	private static PlaySoundResult PlaySoundAtVolume(string sType, 
		...
		do {
			playedState = PlaySoundIfAvailable(sType, randomSource, sourcePosition, volumePercentage, ref forgetSoundPlayedOrScheduled, pitch, _group, sourceTrans, attachToSource, delaySoundTime, useVector3, makePlaySoundResult);
	...
	private static PlaySoundResult PlaySoundIfAvailable(string soundGroupName,
		...
		if (info.source.GetComponent<AudioSource>().isPlaying) {
		...
		info.source.GetComponent<AudioSource>().volume = targetVolume;
		...
		#else
			isActive = info.source.GetComponent<AudioSource>().gameObject.activeInHierarchy;
		#endif
		...
		if (delaySoundTime == 0f) {
			...
			if (ma.EnableMusicDucking && matchingDuck != null) {
				// duck music
				var duckLength = info.source.GetComponent<AudioSource>().clip.length;
	...
	public static void UnpauseSoundGroup(string sType) {
		...
		for (var i = 0; i < sources.Count; i++) {
			...
			if (aVar.GetComponent<AudioSource>().time <= 0f) {
				continue;
			}
			
			aVar.GetComponent<AudioSource>().Play();
		}
	}
		...
		for (var i = 0; i < aGroup.groupVariations.Count; i++) {
			...
			for (var j = 0; j < aVariation.weight; j++) {
				...
				if (aVariation.audLocation == AudioLocation.Clip) {
					clipName = aVariation.GetComponent<AudioSource>().clip == null ? string.Empty : UtilStrings.TrimSpace(aVariation.GetComponent<AudioSource>().clip.name);
				} else {
				...
				case MasterAudio.AudioLocation.Clip:
					clip = aVariation.GetComponent<AudioSource>().clip;
					if (clip == null) {
						Debug.LogWarning("No clip specified in DynamicSoundGroupCreator '" + creatorObjectName + "' - skipping this item.");
						continue;
					}
					variation.GetComponent<AudioSource>().clip = clip;
					break;
				case MasterAudio.AudioLocation.ResourceFile:
					AudioResourceOptimizer.AddTargetForClip(aVariation.resourceFileName, variation.GetComponent<AudioSource>());
					variation.audLocation = MasterAudio.AudioLocation.ResourceFile;
					variation.resourceFileName = aVariation.resourceFileName;
					break;
				}

				variation.original_Pitch = aVariation.GetComponent<AudioSource>().pitch;
